...
const char *test_file = "./tests/data/testfile_buff.store";
const char *fail_file = "./tests/data/failfile_buff.store";
const char *fail_dir = "./tests/data/faildir";
const char *read_file = "./tests/data/readtest_buff.store";
...
START_TEST(create_succeed)
{
    ...
    try {
        test = new BufferedIOHandler(new RawIOHandler(test_file), 10);
    } catch (IOException& e) {
        error = true;
    }
...
START_TEST(create_fail)
{
    ...
    try {
        new BufferedIOHandler(new RawIOHandler(fail_file), 10);
    } catch (IOException& e) {
        error = true;
    }
...
START_TEST(create_fail_nonnorm)
{
    ...
    try {
        new BufferedIOHandler(new RawIOHandler(fail_dir), 10);
    } catch (IOException& e) {
        error = true;
    }
...
START_TEST(read_test)
{
    ...
    test = new BufferedIOHandler(new RawIOHandler(read_file), 10);
